...
class Stage {
	constructor(canvas, playerClass, difficulty, num_enemies=0, num_obstacles=5){
		this.canvas = canvas;
		...
		this.initialise();
		console.log("im gonna start crying!!");
	}
	...
	initialise(num_enemies=5, num_obstacles=10){
		...
		// this.addPlayer(new Soldier(this, position, velocity, "white", radius));

		if(this.playerClass == "Agent") this.addPlayer(new Agent(this, position, velocity, "white", radius));
		if(this.playerClass == "Tricky") this.addPlayer(new Tricky(this, position, velocity, "white", radius));
		if(this.playerClass == "Soldier") this.addPlayer(new Soldier(this, position, velocity, "white", radius));
		if(this.playerClass == "Assassin") this.addPlayer(new Assasin(this, position, velocity, "white", radius));
		...
		for(let i = 0; i < num_enemies + this.difficulty*2; i++){
			this.spawnEnemy(num_enemies + this.difficulty*2);
		}
	...
	spawnCrate(){
		...
		if( crateTotal > 3 + 2 - this.difficulty || Math.random()*5 > 1) return;
...
class Player extends Actor {
	...
	constructor(stage, position, velocity, colour, radius, stride=8) {
		super(stage, position, velocity, colour, radius);
		this.up = false;
		this.down = false;
		this.left = false;
	...
	step(){
		...
		this.stage.playerMove.x += (this.position.x - originalX);
		this.stage.playerMove.y += (this.position.y - originalY);
...
class Tricky extends Player{
	step(){
		if(this.special){
			...
			this.stage.playerMove.x += 2*this.stride*this.lookX;
			this.stage.playerMove.y += 2*this.stride*this.lookY;
...
class Assasin extends Player{
	attack(){
		...
		// velocity.normalize();
		// var bullet = new MeleeBullet(this.stage, startPos, velocity, "white", 1, 3, this, 200);
		// this.stage.addActor(bullet);
...
class NPC extends Actor{
	...
	constructor(stage, position, velocity, colour, radius, stride=4) {
		super(stage, position, velocity, colour, radius);
		this.lookAt = stage.player.position;
		this.count = 1;
		this.maxHealth = 100;
		this.health = this.maxHealth;
		this.stride = stride;
	...
	step(){
		...
		if(this.stage.player != null){
		   var playerPosition = this.stage.player.position;
		}else{
...
class Gun{
	...
	shoot(){
		if(this.owner != null && this.ammo > 0){
			...
			var bullet = new this.bullet(stage, startPos, new Pair(this.owner.lookX, this.owner.lookY), this.color, this.bulletSize, this.duration, this);
			this.stage.addActor(bullet);
		}
...
class QA extends Gun{
	shoot(){
		if(this.owner != null && this.ammo > 0){
			...
			var bullet = new this.bullet(stage, startPos, new Pair(this.owner.lookX, this.owner.lookY), this.color, this.bulletSize*16, this.duration*2, this, 1000);
			this.stage.addActor(bullet);
	...
	shoot(){
		if(this.count <= 0){
			if(this.owner != null && this.ammo > 0){
				...
				var bullet = new this.bullet(stage, startPos, new Pair(this.owner.lookX, this.owner.lookY), this.color, this.bulletSize*2, this.duration*2, this, 5);
				this.stage.addActor(bullet);
...
class Shotgun extends Gun{
	...
	releaseBullet(){
		...
		var bullet = new this.bullet(stage, startPos, velocity, this.color, this.bulletSize, this.duration, this, 20);
		this.stage.addActor(bullet);
	}
}


// class Melee extends Gun{
// 	constructor(bullet){
// 		this.bullet = bullet;
// 	}

// 	shoot(){
// 		this.bullet.checkCollide();
// 	}
// }
...
// 			this.ammo--;		
// 			var startPos = new Pair(this.owner.position.x + this.owner.lookX, this.owner.position.y + this.owner.lookY);
// 			var angleDiffs = [0, -Math.PI/2, Math.PI/2];
// 			for(let i = 0; i < 3; i++){
				

// 				// let diffX = Math.round( - this.position.x);
// 				// let diffY = Math.round( - this.position.y);
				
// 				let angle = Math.atan2(this.owner.lookY, this.owner.lookX);

// 				angle += angleDiffs[i];

// 				let suggestedX = Math.round(Math.cos(angle)*this.owner.radius/2);
// 				let suggestedY = Math.round(Math.sin(angle)*this.owner.radius/2);



// 				let velocity = new Pair(suggestedX, suggestedY);
// 				var bullet = new this.bullet(stage, startPos, velocity, this.color, this.bulletSize, this.duration, this);
// 				this.stage.addActor(bullet);
// 			}
// 		}
// 	}
...
class Bullet extends Actor{
	...
	doDamage(collision){
		...
		if(collision.actor.checkDeath() && this.gun.owner == this.stage.player && collision.actor instanceof NPC){
			score += possibleKill;
			this.stage.kills++;
		}
...
// 	checkCollide(){
// 		try{
// 			this.radius = this.gun.owner.radius*2;
// 		}catch(err){
// 			this.radius = 6;
// 		}
// 		for(let i = 0; i < this.stage.actors.length; i++){
// 			if(this.stage.actors[i] != this.gun.owner && this.singleCollideCheck(this.stage.actors[i])) this.doDamage(new Collision(true, this.stage.actors[i]));
// 		}
// 		this.radius /= 2;
// 	}
...
// 		let score = collision.actor.hitWorth;
// 		let possibleKill = collision.actor.killWorth; 
// 		if(collision.actor.checkDeath() && this.gun.owner == this.stage.player){
// 			score += possibleKill;
// 			this.stage.kills++;
// 		}
// 		this.gun.owner.score += score;

...
void run_kernel3(const int8_t *filter, int32_t dimension, const int32_t *input,
  ...
  kernel3<<<SPLIT, height/SPLIT + 1>>>(d_filter, dimension, d_input, d_output, width, height);
  ...
  normalize3<<<SPLIT, height/SPLIT + 1>>>(d_output, width, height, smallest, largest);
  gpu_time = clock.stop();
  ...
  print_run(cpu_time, 3, gpu_time, transfer_in, transfer_out);
...
__device__ void apply2d3(const int8_t *filter, int32_t dimension,
...
__device__ void normalize_one3(int32_t *image, int32_t width, int32_t height,
...
__global__ void kernel3(const int8_t *filter, int32_t dimension,
  ...
  int start = (height/SPLIT)*blockIdx.x;
  int end;
  if(blockIdx.x > 0){
    end = height;
  }else{
  ...
  if(tid+start < end){
    ...
    for(int j = 0; j < width; j++){
        apply2d3(filter, dimension, input, output, width, height, tid+start, j);
    }
...
__global__ void normalize3(int32_t *image, int32_t width, int32_t height,
  ...
  if(tid+start < end){
    ...
    for(int j = 0; j < width; j++){
        normalize_one3(image, width, height, smallest, biggest, tid+start, j);
    }

...
void run_kernel1(const int8_t *filter, int32_t dimension, const int32_t *input,
  ...
  clock.start();
  // Call the kernel which applies the filter to the image, and puts the end product in d_output
  kernel1<<<width, height>>>(d_filter, dimension, d_input, d_output, width, height);

  unsigned int n; 
  int numThreads, numBlocks, shMemSize, maxBlocks;
  n = width*height;
  // Cant have more blocks than the number of pixels or the actual maximum decided by cuda
  maxBlocks = MY_MIN(MAX_BLOCK_SIZE, n);
  // Calculate num threads (from lab10, i dont get it myself)
  numThreads = (n < 1024*2) ? (n>>1) : 1024;
  // Make sure numThreads is a power of 2
  numThreads = check_threads(numThreads);
  // Calculate num blocks (from lab10, i dont get it myself)
  numBlocks = MY_MIN(maxBlocks, (n+(numThreads*2-1))/(numThreads*2));
  // Calculate shared mem size (from lab10, i dont get it myself)
  ...
  // The reduction produces numblocks output, and we only want one, so w8 till we get one
  while(numBlocks > 1){
    ...
    numBlocks = MY_MIN(maxBlocks, (n+(numThreads*2-1))/(numThreads*2));
    shMemSize = (numThreads <= 32) ? 2*numThreads * sizeof(int32_t) : numThreads * sizeof(int32_t);
    // printf("---new--- n:%d, shMemSize:%d\n", n, shMemSize);
    call_reduce(numThreads, numBlocks, 8*shMemSize, largest, smallest, largest, smallest, n);
  }
  
  // Call the normalize in same pattern as we called the kernel
  normalize1<<<width, height>>>(d_output, width, height, smallest, largest);
  ...
  print_run(cpu_time, 1, gpu_time, transfer_in, transfer_out);

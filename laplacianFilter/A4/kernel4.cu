...
void run_kernel4(const int8_t *filter, int32_t dimension, const int32_t *input,
  ...
  kernel4<<<SPLIT, THREADS>>>(d_filter, dimension, d_input, d_output, width, height);

  unsigned int n; 
  int numThreads, numBlocks, shMemSize, maxBlocks;
  n = (unsigned int) num_elements;
  ...
  normalize4<<<SPLIT, THREADS>>>(d_output, width, height, smallest, largest);
  ...
  print_run(cpu_time, 4, gpu_time, transfer_in, transfer_out);
...
__device__ void apply2d4(const int8_t *filter, int32_t dimension,
...
__device__ void normalize_one4(int32_t *image, int32_t width, int32_t height,
...
__global__ void kernel4(const int8_t *filter, int32_t dimension,
  ...
  int stride = SPLIT * numThreads;
  int start = blockIdx.x*numThreads + threadIdx.x;


  for(int i = start; i < width*height; i+=stride){
    apply2d4(filter, dimension, input, output, width, height, i/width, i%width);
  }
...
__global__ void normalize4(int32_t *image, int32_t width, int32_t height,
  ...
  // We will only have two blocks
  // Start and end represent rows
  int numThreads = blockDim.x;
  int stride = SPLIT * numThreads;
  int start = blockIdx.x*numThreads + threadIdx.x;
  ...
  for(int i = start; i < width*height; i+=stride){
    normalize_one4(image, width, height, smallest, biggest, i/width, i%width);
  }

...
void compare_kernel_against_handwritten_example_1(int kernel) {
  ...
  case 5: {
    run_kernel5(image, output_img.matrix, width,
                height, 0.0);
    break;
  }
  }
...
void compare_kernel_against_handwritten_example_2(int kernel) {
  const int width = 5;
  const int height = 5;

  int32_t image[] = {5, 2, 12, 31, 6,
                     2, 3, 21, 42, 0,
                     1, 4, 22, 75, 133,
                     8, 11, 14, 16, 41,
                     3, 6, 8, 9, 1};
  ...
  switch (kernel) {
    ...
    case 5: {
      run_kernel5(image, output_img.matrix, width, height, 0.0);
      break;
    }
  }
  ...
  int32_t expected[] = {35, 18, 27, 63, 15,
                            30, 19, 0, 0, 0};
...
TEST(csc367a4, cpu_best_handwritten_tests) {
...
TEST(csc367a4, compare_kernels_against_each_other) {
  ...
  for (int width = 1; width < WIDTH_MAX; ++width) {
    for (int height = 1; height < HEIGHT_MAX; ++height) {
      ...
      int split = check_threads(width*height);
      ...
      run_kernel4(FILTER, FILTER_DIMENSION, source.matrix, outputs[4].matrix,
                  width, height, 0.0);
      run_kernel5(source.matrix, outputs[5].matrix,
                  width, height, 0.0);
